# Nama File: vulnera_dashboard_complete_enhanced_final_FIXED_v6.R
# Dashboard Analisis Kerentanan Sosial - VERSI FINAL TANPA ERROR
...
# 5. FUNGSI SPATIAL SETUP - PERBAIKAN LENGKAP
...
# 6. Generate data dan setup spatial - PERBAIKAN
...
calculate_sovi <- function(data) {
  # Variabel yang berkontribusi positif terhadap kerentanan
  ...
  # Variabel yang berkontribusi negatif terhadap kerentanan
  ...
  for (var in negative_vars) {
    if (var %in% names(data)) {
      data_normalized[[paste0(var, "_z")]] <- -scale(data[[var]])[,1]  # Negatif karena mengurangi kerentanan
    }
  }
...
server <- function(input, output, session) {
  ...
  # Interactive Map Output - CHOROPLETH MAP DENGAN PERBAIKAN LENGKAP
  output$choropleth_map <- renderLeaflet({
    ...
    tryCatch({
      ...
      if (is.null(sf_data) || input$map_type == "points") {
        ...
        # Choropleth map dengan GeoJSON
        # Merge data dengan spatial data berdasarkan nama wilayah
        ...
        if (nrow(merged_data) == 0) {
          # Fallback ke point map jika merge gagal
          pal <- colorNumeric(palette = input$color_palette, domain = data[[input$map_indicator]])
  ...
  output$scatter_matrix <- renderPlot({
    ...
    if (length(selected_vars) > 6) {
      selected_vars <- selected_vars[1:6]  # Batasi maksimal 6 variabel
    }
